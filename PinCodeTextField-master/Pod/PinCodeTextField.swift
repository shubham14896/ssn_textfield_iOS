    ...
    private func updateLabels() {
        ...
        for (i,label) in labels.enumerated() {
            ...
            if isSecureTextEntry {
                let firstChar = currentCharacter.map { String($0) }
                
                label.text = firstChar
                
                let textCount = (text?.count ?? 0) - 1
                
                let delay = (i == textCount || textCount == 3 || textCount == 6 ) ? 0.3 : 0
                
                DispatchQueue.main.asyncAfter(deadline: .now() + delay) {
                    label.text = firstChar == "-" ? "-" : "x"
                }
            } else {
...
extension PinCodeTextField: UIKeyInput {
    ...
    public func insertText(_ charToInsert: String) {
        if charToInsert.hasOnlyNewlineSymbols {
            ...
            if newText.count == 3 || newText.count == 6 {
                print("ADD NEW -")
                text = newText + "-"
            }
